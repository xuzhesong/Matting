function alpha = runLaplacian(RGBImagePath, TrimapImagePath, levels_num, active_levels_num)
    ...
    I=double(imread(RGBImagePath))/255;
    mI=double(imread(TrimapImagePath))/255;
    consts_map=sum(abs(I-mI),3)>0.001;
    if (size(I,3)==3)
      consts_vals=im2gray(mI).*consts_map;
    end
    if (size(I,3)==1)
